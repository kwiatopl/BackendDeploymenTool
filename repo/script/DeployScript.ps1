...
function MailLog($log)
{
    ...
    $msg.From = "Logs@DeployScript.com"
    $msg.To.Add("support.search@volvo.com") #CHANGE IT TO support.search@volvo.com !!
...
function CheckContentSources($nodes)
{
    ...
    {
        $app = ''
        LogWrite -logMessage ("Loading SP Service Application '{0}'" -f $node.SearchApplication) -logFile $logFile
        try 
        {
        ...
        {
            LogWrite -logMessage ("### ERROR ### Failed to load SP Service Application with exception message: '{0}'" -f $_.ExceptionMessage) -logFile $logFile
        }
        
        if(-Not $app)
        {
            LogWrite -logMessage ("Fail to load SP Service Application '{0}'" -f $node.SearchApplication) -logFile $logFile
        }
        else
        {    
            LogWrite -logMessage ("Loaded SP Service Application '{0}'" -f $node.SearchApplication) -logFile $logFile
            $actualContentSources = ''
            LogWrite -logMessage ("Loading Content Source '{0}'" -f $node.Name) -logFile $logFile
            try 
            {
            ...
            {
                LogWrite -logMessage ("### ERROR ### Failed to load Content Sources from SSA with exception message: '{0}'" -f $_.ExceptionMessage) -logFile $logFile
            }
            if(-Not $actualContentSources)
            {
                LogWrite -logMessage ("Failed to load Content Sources from SP Service Application: '{0}'" -f $node.SearchApplication) -logFile $logFile
            }
            else 
            {
                ...
                {
                    LogWrite -logMessage ("Comparing Content Source from XML: '{0}' with Content Source from SP App: '{1}'" -f $node.Name, $cs.Name) -logFile $logFile
...
{
    LogWrite -logMessage ("Stepped into AddContentSource function") -logFile $logFile
    LogWrite -logMessage ("Content Source '{0}' received in AddContentSource function" -f $node.Name) -logFile $logFile
    $name = $node.Name
    $maxPage = ZeroCheck $node.MaxPageEnum erationDepth
    ...
    if($node.Type -eq 'Business')   # FOR BUSINESS TYPE CONTENT SOURCE
    {
        ...
        if($proxyGroup -eq '')
        {
            ...
            {
                LogWrite -logMessage ("### ERROR ### {0}: Failed to load default proxy group with exception message: '{1}'" -f $node.Name, $_.ExceptionMessage) -logFile $logFile
            }
        }
    ...
    try
    {
        $newContentSource = New-SPEnterpriseSearchCrawlContentSource -Name $node.Name -SearchApplication $app -Type $node.Type -StartAddresses $node.StartAddresses -SharePointCrawlBehavior $node.SelectSingleNode("//CrawlSettings").SharepointCrawlBehavior -MaxPageEnumerationDepth $maxPage -MaxSiteEnumerationDepth $maxSite -BDCApplicationProxyGroup $proxyGroup -LOBSystemSet $lobSystems -CrawlPriority $node.SelectSingleNode('//CrawlSettings').CrawlPriority
    }
    catch
    {
        LogWrite -logMessage ("### ERROR ### {0}: Failed to create Content Source with exception: '{1}'" -f $node.Name, $_.ExceptionMessage) -logFile $logFile
    }
    if(-Not $newContentSource)
    {
        LogWrite -logMessage ("{0}: Failed to create Content Source" -f $node.Name) -logFile $logFile
    }
    else
    {
        ...
        {
            LogWrite -logMessage ("### ERROR ### {0}: Failed to get Content Source for Continuous Crawl check with exception: '{1}'" -f $node.Name, $_.ExceptionMessage) -logFile $logFile
        }
        ...
        if($node.SelectSingleNode('//CrawlSettings').ContinuousCrawl -eq 1) 
        {
            ...
            try
            {
                $continuousCrawl = Set-SPEnterpriseSearchCrawlContentSource -Identity $acs -EnableContinuousCrawls $true
            }
            catch
            {
                LogWrite -logMessage ("### ERROR ### {0}: Failed to set Continuous Crawl for Content Source with exception: '{1}'" -f $node.Name, $_.ExceptionMessage) -logFile $logFile
            }
            if(-Not $continuousCrawl)
            {
                LogWrite -logMessage ("{0}: There is some problem with setting Continuous Crawl" -f $node.Name) -logFile $logFile
            }
            else 
            {
                try 
                {
                    $continuousCrawl.Update()
                }
                catch 
                {
                    LogWrite -logMessage ("### ERROR ### {0}: Failed to update Content Source with Continuous Crawl setting with exception: '{1}'" -f $node.Name, $_.ExceptionMessage) -logFile $logFile
                }
                LogWrite -logMessage ("{0}: Continouos Crawl enabled" -f $node.Name) -logFile $logFile
            }
        }
...
{ 
    LogWrite -logMessage ("{0}: Stepped into CrawlSchedule function"-f $node.Name) -logFile $logFile
    ...
    foreach($singlenode in $node.SelectNodes("//ContentSource[@Name='$name']/CrawlScheduleList/CrawlSchedule"))      #FOR EACH CRAWL SCHEDULE IN XML FILE
    {
        ...
        if($singlenode.ScheduleType -ne '' -and $numerator -le 2)
        {
            switch($singlenode.CrawlScheduleType)
            {
                Daily
                {
                    ...
                    {
                        LogWrite -logMessage ("### ERROR ### {0}: Failed to set Crawl Schedule for Content Source with exception: '{1}'" -f $node.Name, $_.ExceptionMessage) -logFile $logFile
                    }
        ...
        elseif($singlenode.ScheduleType -eq '')
        {
            ...
            {
                LogWrite -logMessage ("### ERROR ### {0}: Failed to get actual Crawl Schedule from Content Source with exception: '{1}'" -f $node.Name, $_.ExceptionMessage) -logFile $logFile
            }
...
{
    LogWrite -logMessage ("Stepped into EditContentSource function") -logFile $logFile
    ...
    {
        LogWrite -logMessage ("### ERROR ### {0}: Failed to get Crawl Schedules from Content Source to edit with exception: '{1}'" -f $node.Name, $_.ExceptionMessage) -logFile $logFile
    }
    if(-Not $actualCrawlSchedule)
    {
        LogWrite -logMessage ("{0}: Failed to load actual Crawl Schedules" -f $csToEdit.Name) -logFile $logFile
    }
    else 
    {
...
function ManageCrawlRules($nodes)
{
    ...
    {    
        ...
        if($crawlrule.AccountPassword -ne '')
        {
            ...
            {
                LogWrite -logMessage ("### ERROR ### {0}: Failed to create secure string for password with exception: '{1}'" -f $node.Name, $_.ExceptionMessage) -logFile $logFile
            }
        ...
        if($crawlrule.Path -ne '')
        {
            ...
            LogWrite -logMessage ("{0}: Loading Search Application {1}" -f $crawlrule.Path, $crawlrule.SearchApplication) -logFile $logFile
            try 
            {
            ...
            {
                LogWrite -logMessage ("### ERROR ### {0}: Failed to get Search Service Application with exception: '{1}'" -f $node.Name, $_.ExceptionMessage) -logFile $logFile
            }
            if(-Not $app)
            {
                LogWrite -logMessage ("{0}: Failed to load Search Application {1}" -f $crawlrule.Path, $crawlrule.SearchApplication) -logFile $logFile
            }
            else 
            {
                LogWrite -logMessage ("{0}: Loaded Search Application {1}" -f $crawlrule.Path, $crawlrule.SearchApplication) -logFile $logFile
            }
            LogWrite -logMessage ("{0}: Loading Crawl Rules for Search Application {1}" -f $crawlrule.Path, $crawlrule.SearchApplication) -logFile $logFile
            ...
            {
                LogWrite -logMessage ("### ERROR ### {0}: Failed to get Crawl Rules for SSA with exception: '{1}'" -f $node.Name, $_.ExceptionMessage) -logFile $logFile
            }
            
            if(-Not $acr)
            {
                LogWrite -logMessage ("{0}: Failed to load Crawl Rules for Search Application {1}" -f $crawlrule.Path, $crawlrule.SearchApplication) -logFile $logFile
            }
            else 
            {
            ...
            if($acr -ne $null -and $app -ne $null)
            {
                ...
                {
                    LogWrite -logMessage ("{0}: Checking if Crawl Rule already exists" -f $crawlrule.Path) -logFile $logFile
                    if($rule.Path -eq $crawlrule.Path)
                    {
                        ...
                        {
                            LogWrite -logMessage ("### ERROR ### {0}: Failed to delete Crawl Rule with exception: '{1}'" -f $node.Name, $_.ExceptionMessage) -logFile $logFile
                        }
                ...
                {
                    LogWrite -logMessage ("### ERROR ### {0}: Failed to stage adding of new Crawl Rule with exception: '{1}'" -f $node.Name, $_.ExceptionMessage) -logFile $logFile
                }
            }
            if($ruleToAdd -ne $null)    # IF RULE IS CORRECTLY ASSIGNED
            {
                ...
                {
                    LogWrite -logMessage ("### ERROR ### {0}: Failed to commit adding of Crawl Rule with exception: '{1}'" -f $node.Name, $_.ExceptionMessage) -logFile $logFile
                }
...
function SearchSchema($nodeSchema)
{
    ...
    foreach($node in $nodeSchema.SelectNodes('//ManagedProperty'))  # FOREACH MANAGED PROPERTY IN XML
    {
        ...
        {
            LogWrite -logMessage ("### ERROR ### {0}: Failed to get Managed Properties with exception: '{1}'" -f $node.Name, $_.ExceptionMessage) -logFile $logFile
        }
        if($managedProperty -eq $null)  # IF NOT FOUND THE MANAGED PROPERTY
        {
            ...
            try
            {
                $managedProperty = New-SPEnterpriseSearchMetadataManagedProperty -Name $node.Name -SearchApplication $node.SearchApplication -Type $node.Type -Retrievable ([int]$node.Retrievable -as [bool]) -SafeForAnonymous ([int]$node.SafeForAnonymous -as [bool]) -RespectPriority ([int]$node.RespectPriority -as [bool]) -NameNormalized ([int]$node.NameNormalized -as [bool]) -EnabledForScoping ([int]$node.EnabledForScoping -as [bool]) -FullTextQueriable ([int]$node.FullTextQueriable -as [bool]) -NoWordBreaker ([int]$node.NoWordBreaker -as [bool]) -RemoveDuplicates([int]$node.RemoveDuplicates -as [bool]) -ErrorAction SilentlyContinue   
            }
            catch 
            {
                LogWrite -logMessage ("### ERROR ### {0}: Failed to stage creation of new Managed Property with exception: '{1}'" -f $node.Name, $_.ExceptionMessage) -logFile $logFile
            }
            if(-Not $managedProperty)
            {
                LogWrite -logMessage ("{0}: Failed to create Managed Property" -f $node.Name) -logFile $logFile    
            }
            else 
            {
                ...
                {
                    LogWrite -logMessage ("### ERROR ### {0}: Failed to commit the creation of new Managed Property with exception: '{1}'" -f $node.Name, $_.ExceptionMessage) -logFile $logFile
                }
                LogWrite -logMessage ("{0}: Created Managed Property with parameters" -f $node.Name) -logFile $logFile
                LogWrite -logMessage ("{0}: {1}" -f $node.Name, $managedProperty) -logFile $logFile
            }
        }
        elseif([int]$node.OnlyCrawledProperties -as [bool] -ne $true) 
        {
            ...
            if($name -ne $managedProperty.Name -as [bool] -or $managedProperty.Retrievable -ne [int]$node.Retrievable -as [bool] -or $managedProperty.Queryable -ne [int]$node.Queryable -as [bool] -or $managedProperty.SafeForAnonymous -ne [int]$node.SafeForAnonymous -as [bool] -or $managedProperty.RespectPriority -ne [int]$node.RespectPriority -as [bool] -or $managedProperty.NameNormalized -ne [int]$node.NameNormalized -as [bool] -or $managedProperty.Sortable -ne [int]$node.Sortable -as [bool] -or $managedProperty.Refinable -ne [int]$node.Refinable -as [bool] -or $managedProperty.Searchable -ne [int]$node.Searchable -as [bool] -or $managedProperty.HasMultipleValues -ne [int]$node.HasMultipleValues -as [bool] -or $managedProperty.Sortable -ne [int]$node.Sortable -as [bool] -or $managedProperty.Refinable -ne [int]$node.Refinable -as [bool] -or $managedProperty.HasMultipleValues -ne [int]$node.HasMultipleValues -as [bool] -or [string]$managedProperty.SortableType -ne $node.SortableType -or [string]$managedProperty.RefinerConfiguration.Type -ne $node.RefinerType)
            {
                ...
                else 
                {
                    try 
                    {
                        Set-SPEnterpriseSearchMetadataManagedProperty -SearchApplication $node.SearchApplication -Identity $node.Name -Retrievable ([int]$node.Retrievable -as [bool]) -SafeForAnonymous ([int]$node.SafeForAnonymous -as [bool]) -RespectPriority ([int]$node.RespectPriority -as [bool]) -NameNormalized ([int]$node.NameNormalized -as [bool]) -EnabledForScoping ([int]$node.EnabledForScoping -as [bool]) -FullTextQueriable ([int]$node.FullTextQueriable -as [bool]) -NoWordBreaker ([int]$node.NoWordBreaker -as [bool]) -RemoveDuplicates([int]$node.RemoveDuplicates -as [bool]) -ErrorAction SilentlyContinue   
                    }
                    catch 
                    {
                        LogWrite -logMessage ("### ERROR ### {0}: Failed to stage editing of Managed Property with exception: '{1}'" -f $node.Name, $_.ExceptionMessage) -logFile $logFile
                    }
                    $managedProperty.Queryable = [int]$node.Queryable -as [bool]
                    $managedProperty.Searchable = [int]$node.Searchable -as [bool]
                    $managedProperty.Sortable = [int]$node.Sortable -as [bool]
                    ...
                    {
                        LogWrite -logMessage ("### ERROR ### {0}: Failed to commit edit of Managed Property with exception: '{1}'" -f $node.Name, $_.ExceptionMessage) -logFile $logFile
                    }
        ...
        } elseif([int]$node.OnlyCrawledProperties -as [bool])
        {
        ...
        foreach($nodeProperty in $crawledPropertiesXML) # FOREACH ADD CRAWL PROPERTY IN XML
        {
            ...
            {
                LogWrite -logMessage ("### ERROR ### {0}: Failed to add Crawled Properties to be mapped to Managed Property with exception: '{1}'" -f $node.Name, $_.ExceptionMessage) -logFile $logFile
            }
        }
        ...
        if($crawledProperties -ne $null -and $managedProperty -ne $null)
        {
            ...
            {
                LogWrite -logMessage ("### ERROR ### {0}: Failed to add mapping Crawled Properties to Managed Property with exception: '{1}'" -f $node.Name, $_.ExceptionMessage) -logFile $logFile
            }
        }
        ...
        foreach($nodeProperty in $crawledPropertiesToRemoveXML) # FOREACH REMOVE CRAWL PROPERTY IN XML
        {
            ...
            {
                LogWrite -logMessage ("### ERROR ### {0}: Failed to remove Crawled Properties to map to Managed Property with exception: '{1}'" -f $node.Name, $_.ExceptionMessage) -logFile $logFile
            }
        }
        ...
        if($crawledPropertiesToRemove -ne $null -and $managedProperty -ne $null)
        {
            try 
            {
                for($i=0; $i -le $crawledPropertiesToRemove.Count-1; $i++)
                {
                    ...
                    # REMOVE OF MAPPING
                    Remove-SPEnterpriseSearchMetadataMapping -Identity $mapToRemove -SearchApplication $node.SearchApplication -ErrorAction SilentlyContinue -Confirm:$false
                    LogWrite -logMessage ("{0}: Removed mapping of Crawled Property {1}" -f $node.Name, $crawledPropertiesToRemove[$i].Name) -logFile $logFile
                }
            }
            catch 
            {
                LogWrite -logMessage ("### ERROR ### {0}: Failed to remove mapping of Crawled Properties with exception: '{1}'" -f $node.Name, $_.ExceptionMessage) -logFile $logFile    
            } 
        }
        LogWrite -logMessage ("{0}: Removing completed" -f $node.Name) -logFile $logFile
    }
}
...
function ResultSource($nodes)
{
    ...
    foreach($node in $nodes) #FOREACH RESULT SOURCE IN RESULT SOURCES
    {
        ...
        {
            LogWrite -logMessage ("### ERROR ### {0}: Failed to add/modify Result Source with exception: '{1}'" -f $node.Name, $_.ExceptionMessage) -logFile $logFile    
        }
        
    }
}

function RunCews($nodes)
{
    try 
    {
        LogWrite -logMessage ("Stepped into RunCews function") -logFile $logFile
        if(($nodes.Body -ne '' -or $nodes.Body -ne $null) -and ($nodes -ne '' -or $nodes -ne $null))
        {
            $script = $nodes.Body
            LogWrite -logMessage ("Script that is about to run: {0}" -f $nodes.Body) -logFile $logFile
            if([string]::IsNullOrEmpty($nodes.Body)) {
                LogWrite -logMessage ("Script is empty") -logFile $logFile}
            else
            {
                Invoke-Expression $script
                LogWrite -logMessage ("Script completed") -logFile $logFile
            }
        }
        LogWrite -logMessage ("Stepped out of RunCews function") -logFile $logFile
    }
    catch 
    {
        LogWrite -logMessage ("### ERROR ### Failed to RunCews with exception: '{0}'" -f $_.ExceptionMessage) -logFile $logFile   
    }
}
...
function Crawl ($nodes, $time)
{
    try 
    {
        ...
        {
            $name = $node.Name
            if($node.Name -ne '' -and $node.SelectSingleNode('//CrawlSettings').Crawl -ne '')
            {
                LogWrite -logMessage ("{0}: Loading Content Source to add for Crawl" -f $node.Name) -logFile $logFile
                $csToCrawl = Get-SPEnterpriseSearchCrawlContentSource -SearchApplication $node.SearchApplication -Identity $node.Name -ErrorAction SilentlyContinue   # GET CONTENT SOURCE
                if(-Not $csToCrawl)
                {
                ...
                else 
                {
                    ...
                    {
                        if($node.SelectSingleNode("//ContentSource[@Name='$name']//CrawlSettings").Crawl -eq 'Full')
                        {
                            LogWrite -logMessage ("{0}: Crawl State of Content Source is Idle" -f $node.Name) -logFile $logFile
                            LogWrite -logMessage ("{0}: Starting Full Crawl" -f $node.Name) -logFile $logFile
                            $csToCrawl.StartFullCrawl() # START FULL CRAWL
                        }
                        elseif($node.SelectSingleNode("//ContentSource[@Name='$name']//CrawlSettings").Crawl -eq 'Incremental')
                        {
                            LogWrite -logMessage ("{0}: Crawl State of Content Source is Idle" -f $node.Name) -logFile $logFile
            ...
            elseif($node.SelectSingleNode('//CrawlSettings').Crawl -eq '' -or $node.SelectSingleNode('//CrawlSettings').Crawl -eq $null)
            {
                LogWrite -logMessage ("{0}: No Full or Incremental Crawl set" -f $node.Name) -logFile $logFile
            }
            else
            {
    ...
    {
        LogWrite -logMessage ("### ERROR ### Failed to start Thread Sleep for Crawl Run with exception: '{0}'" -f $_.ExceptionMessage) -logFile $logFile  
    }
...
try
{
    ...
    $logFile = $logPath + $date + "_DeployScriptLog.txt"
    ...
    <############################ CRAWL ##########################################> 
    if($xmlDoc.SelectSingleNode('//RunCrawl') -eq '1')
    {
    ...
    <######################### CEWS ######################################>
    RunCews $xmlDoc.SelectNodes("//CEWS")
    LogWrite -logMessage ("Stepped out of RunCews function") -logFile $logFile
    <##########################  FULL CRAWL ############################>
    if($xmlDoc.SelectSingleNode('//RunCrawl') -eq '1')
    {
        [Nullable[int]]$seconds = $null
        Crawl $xmlDoc.SelectNodes("//ContentSource") $seconds
        LogWrite -logMessage ("Stepped out of Crawl function") -logFile $logFile
    }
}
catch
{
